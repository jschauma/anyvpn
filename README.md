...
SYNOPSIS
     anyvpn [-Vhlv] [-m method] [-p app] [-s site] [-u user] on|off|sites|vars
...
COMMANDS
     The following commands are supported by anyvpn:

     on	     Connect to the given VPN site.

     off     Disconnect from the VPN.

     sites   List the available VPN sites you can connect to.

     vars    Print out the value of all relevant environment variables used by
	     anyvpn.
...
ENVIRONMENT
     ...
     PM_VPN_ENTRY	  The name of the password manager entry for your VPN
			  ...
			  KEYCHAIN_VPN_ENTRY, LASTPASS_VPN_ENTRY, or
			  ONEPASS_VPN_ENTRY environment variables.
